...
namespace OpenFluency.Repositories
{
    ...
    public class TurmaRepository : BaseRepository, ITurmaRepository
    {
        ...
        public int? Atualizar(Turma turma)
        {
            using (var conn = new MySqlConnection(ConnectionString))
            {
                string query = "UPDATE turma SET ano = @ano, semestre = @semestre, periodo = @periodo, nivel = @nivel, professor_id = @professor_id WHERE turma_id = @turma_id";
                ...
                cmd.Parameters.AddWithValue("@nivel", turma.Nivel);
                cmd.Parameters.AddWithValue("@professor_id", turma.ProfessorId);
                cmd.Parameters.AddWithValue("@turma_id", turma.ProfessorId);
